...
def _safe_text(value, fallback: str = "") -> str:
    """Return a string suitable for a ReportLab Paragraph."""
    if value is None:
        return fallback
...
def build_pdf(json_path, output_dir=None, filename=None):
    ...
    # --- Thread-safe page callbacks that capture *data* explicitly ---
    ...
    plain_heading_style = ParagraphStyle(
        ...
        textColor=colors.HexColor("#0c1b33"), spaceBefore=12, spaceAfter=3,
        alignment=TA_CENTER,
    )
    numbered_style = ParagraphStyle(
        "numbered", fontSize=10, leading=14, leftIndent=20,
        firstLineIndent=-10, spaceBefore=3, spaceAfter=3,
    )
    heading_style = ParagraphStyle(
        name="Heading", fontSize=30, leading=30, alignment=TA_LEFT,
        spaceAfter=6, spaceBefore=6, textColor=colors.HexColor("#0c1b33"),
        leftIndent=-16,
    )
    ...
    def _generate_pdf(out_path, include_general_instructions=True):
        doc = BaseDocTemplate(
            ...
            rightMargin=20, leftMargin=20, topMargin=20, bottomMargin=60,
        )
        ...
        patient_rows = [
            [
                ...
                Paragraph(_safe_text(pi.get("name")), field_style),
                Paragraph("<b>Age:</b>", label_style),
                Paragraph(_safe_text(pi.get("age")), field_style),
                Paragraph("<b>Gender:</b>", label_style),
                Paragraph(_safe_text(pi.get("gender")), field_style),
            ],
            [
                Paragraph("<b>Date:</b>", label_style),
                Paragraph(_safe_text(pi.get("date")), field_style),
                "", "", "", "",
            ],
        ]
        ...
        recs = data.get("non_pharmacological_recommendations") or []
        recs_paragraphs = []
        for rec in recs:
        ...
        tests = data.get("medical_tests") or []
        tests_paragraphs = []
        for test in tests:
    ...
    # Generate with general instructions first
    _generate_pdf(temp_filename, include_general_instructions=True)

    # Check page count by counting PDF page objects in the raw bytes
    try:
